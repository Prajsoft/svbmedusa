...
export type ReturnCloseWorkflowInput = {
  order_id: string
  return_id?: string
...
export type ReturnCloseWorkflowResult = {
  order_id: string
  return_id: string
  from_state: string
  to_state: string
...
export async function returnCloseWorkflow(
  scope: { resolve: (key: string) => any },
  input: ReturnCloseWorkflowInput
): Promise<ReturnCloseWorkflowResult> {
  ...
  await emitReturnEvent(scope, "return.closed", {
    order_id: order.id,
    return_id: returnId,
    actor_id: input.actor_id,
