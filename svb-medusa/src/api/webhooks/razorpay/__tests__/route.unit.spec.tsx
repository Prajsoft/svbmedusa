...
import { POST as razorpayWebhookRoute } from "../route"
import {
...
describe("razorpay webhook compatibility route", () => {
  ...
  it("delegates to shared pipeline and applies captured transition", async () => {
    ...
    const { req, paymentModule, logger } = makeReq({
      ...
      headers: {
        ...
        "x-razorpay-event-id": "evt_compat_transition_1",
      },
    })
    const res = makeRes()

    await razorpayWebhookRoute(req, res)
    ...
    expect(res.payload).toEqual(
      expect.objectContaining({
        ...
        deduped: false,
        provider: "razorpay",
        event_id: "evt_compat_transition_1",
    ...
    expect(res.headers["x-webhook-endpoint-deprecated"]).toBe("true")
    expect(res.headers["x-webhook-endpoint-canonical"]).toBe(
  ...
  it("dedupes repeated webhook deliveries", async () => {
    const body = buildBody({
    ...
    const { req, paymentModule, logger } = makeReq({
      ...
      headers: {
        "x-razorpay-signature": signature,
        "x-razorpay-event-id": "evt_compat_dedup_1",
      },
    })
    const firstRes = makeRes()
    const secondRes = makeRes()

    await razorpayWebhookRoute(req, firstRes)
    await razorpayWebhookRoute(req, secondRes)
  ...
  it("returns SIGNATURE_INVALID for tampered signatures", async () => {
    const body = buildBody()
    const { req } = makeReq({
      body,
      headers: {
        "x-razorpay-signature": "bad_signature",
        "x-razorpay-event-id": "evt_compat_sig_1",
      },
    })
    const res = makeRes()
  ...
  it("emits shared webhook observability events", async () => {
    const body = buildBody({
      event: "payment.authorized",
    ...
    const { req, logger } = makeReq({
      ...
      headers: {
        "x-razorpay-signature": signature,
        "x-razorpay-event-id": "evt_compat_obs_1",
      },
    })
    const res = makeRes()
